...
#compileBitcode选项没有配置,默认compileBitcode为YES,要设置成NO就可以了
compileBitcode="NO"
...
# 蒲公英app key
api_key="xxxx"
#uKey = "79cc50e258b710913e02cce124e0ffdd"
...
then
    ipa_file="${export_ipa_path}/${ipa_name}.ipa"
    echo "开始上传ipa到蒲公英:${ipa_file}"
        
    curl -F "file=@${ipa_file}" -F "_api_key=${api_key}"  'https://www.pgyer.com/apiv2/app/upload'
#    curl -F "file=@${export_ipa_path}/${ipa_name}.ipa" -F "_api_key=${api_key}"  'https://www.pgyer.com/apiv2/app/upload'
else
    echo "在目录：${export_ipa_path}/${ipa_name}.ipa 不存在"
fi
